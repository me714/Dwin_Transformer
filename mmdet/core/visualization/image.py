...
def imshow_det_bboxes(img,
    ...
    """
    swin_cls = ['wt', 'jgc', 'lsd', 'lxb', 'bbt', 'xgg', 'txd', 'lwz', 'tc', 'xbs', 'wbs', 'a-pg', 'b-pg', 'c-pg',
                'g-pg',
                'z-pg']
    class_names=swin_cls
    ...
    for i, (bbox, label) in enumerate(zip(bboxes, labels)):
        count+=1
        bbox_list.append(bbox[0:4].tolist())
        source_list.append(bbox[4].tolist())
    ...
    plt.text(0, 50, f"{count}", fontsize=30,  color='red')
    plt.imshow(img)
    ...
    return img,bbox_list,source_list,label_list
