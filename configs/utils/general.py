...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    lcls, lbox, lobj, lcot = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1,
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            lbox += (1.0 - iou).mean()  # iou loss   框重合损失

            # 预测的box数量
            # num_count = torch.Tensor([pbox.shape[0]])
            # ans_count = torch.Tensor([tbox[i].shape[0]])

            lcot += torch.Tensor([abs(tbox[i].shape[0] - pbox.shape[0]) / tbox[i].shape[0]]).to(device)

            # Objectness
            tobj[b, a, gj, gi] = (1.0 - model.gr) + model.gr * iou.detach().clamp(0).type(tobj.dtype)  # iou ratio

            # Classification
            if model.nc > 1:  # cls loss (only if multiple classes)   #  种类损失
        ...
        lobj += BCEobj(pi[..., 4], tobj) * balance[i]  # obj loss   置信度损失
    ...
    loss = lbox + lobj + lcls + lcot
    return loss * bs, torch.cat((lbox, lobj, lcls, loss)).detach(), lcot
...
def iou_check(det, threshold=0.93):
    '''
    det shape:nx6 (x1, y1, x2, y2, conf, cls)
    delete iou>threshold box
    Arguments:
    box1 (Tensor[N, 4])
    box2 (Tensor[M, 4])
    Returns:
    ...
    for i, box in enumerate(boxes):
        for k, com in enumerate(boxes):
            if i != k:
                iou = bbox_iou(boxes[i], boxes[k])
                if iou > threshold:
                    boxConfi, boxConfk = det[i, 4], det[k, 4]
                    wd.append(i) if boxConfi < boxConfk else wd.append(k)
    det = det.cpu().detach().numpy()
    det = torch.from_numpy(np.delete(det, wd, axis=0))
    return det
